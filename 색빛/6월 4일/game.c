		...
		if (_kbhit()) // 키 입력 여부
		{
			...
			else if (ch == ENTER) // ENTER: 입력 결과 제출
			{
				...
				if (!strcmp(word, input)) { // 정답
					...
					if (level == T)
					{
						add_test_score(grade, T_CORRECT, score);
						add_test_score(grade, T_TIME, TimeBonus(timeleft));
						if (O_X[SCORE[grade][T_Q_SOLVED] - 1])
							add_test_score(grade, T_COMBO, BonusPerCombo);
						SCORE[grade][T_Q_CORRECT]++;
					}
					else
					{
						addscore(level, CORRECT, score);
						addscore(level, TIME, TimeBonus(timeleft));
						if (O_X[SCORE[level][Q_SOLVED] - 1])
					...
					if (level == T)
					{
						if (combo_print(SCORE[grade][T_Q_SOLVED]) != 0)
							printf("%d COMBO", combo_print(SCORE[grade][T_Q_SOLVED]));
					}
					else
					{
			...
			{
				cursor(0);
				gotoxy(30, 31); printf("[입력 가능 문자: 알파벳 대/소문자, 숫자, 특수기호(\" \' , . ?)]");
			}
		}
	}
	return result;
