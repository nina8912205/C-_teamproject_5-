...
void e_boss() //############ 색빛 - 원래 두 함수였는데 하나로 만듦.
{
	...
	for (int i = 1; i <= 5; i++)
	{
		...
		drawtopbar();
		gotoxy(76, 10);
		printf("│ SCORE│ %010d", TotalScore);//점수 알고리즘 수정********5/26현욱
		gotoxy(25, 10);
		printf(" 초등학교 졸업 시험 - 문제 %d 번\n", i);

		//변수값 확인용//****5/26현욱
		gotoxy(25, 13);
		printf("total %d / correct %d / time bonus %d %d", e_score[boss_total_score], e_score[boss_correct_score], e_score[boss_time_bonus], e_score[boss_combo_bonus]);
