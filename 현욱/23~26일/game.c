...
int time_left; // 남은 시간 저장 변수

// 단어 입력 받기 & 제한 시간이 지나면 넘어가기 -> 결과 반환(0: 오답, 1: 정답, 2: 메뉴 선택)
int game(int size, int timelimit, char* input, char* word, int num_Q) //**** 5/24(현욱)-실시간 콤보 표시위한 인수 추가
{
	char ch;
	...
	while (1)
	{
		...
		if (j >= size) // 문자열의 길이 넘으면	
		{
			...
			//**** 5/24(현욱) - 콤보 갯수 출력 위해 Sleep(1500), System("cls") 이동
			Sleep(1500);
			system("cls");
			break;
		}
