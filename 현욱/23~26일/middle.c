...
void middle(int n) // middle(0): 처음시작, middle(1): 처음아님.
{
	...
	extern int total_score, m_total_score, m_correct_score, m_time_bonus, m_combo_bonus, mboss_total_score;
	...
	for (grade = 1; grade <= 3; grade++)
	{
		...
		{
			gotoxy(32, 24);
			printf("total: %d, e_time: %d", total_score, m_time_bonus);
			gotoxy(32, 25);
			printf("m_correct: %d, m_combo: %d", m_correct_score, m_combo_bonus);
			M_num_Q++;
			// 레이아웃
			drawline();
			...
			while (1)
			{
				if (result == 1)
				{
					time_bonus(MIDDLE, TimeBonus(time_left));
					correct_score(MIDDLE, score);
					if (O_X[M_num_Q - 2] == 1)
					{
						combo_bonus(MIDDLE);
					}
