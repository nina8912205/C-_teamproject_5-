...
void elementary(int n) // elementary(0): 처음시작, elementary(1): 처음아님.
{
	...
	extern int time_left, scorearray[GRADESIZE][SSIZE], TotalScore;
	...
	for (grade = 1; grade <= 6; grade++)
	{
		...
		{
			gotoxy(32, 24);
			printf("total: %d, e_time: %d", TotalScore, scorearray[E][TIME]);
			gotoxy(32, 25);
			printf("e_correct: %d, e_combo: %d", scorearray[E][CORRECT], scorearray[E][COMBO]);
			...
			result = game(grade, timelimit, input, word, scorearray[E][Q_SOLVED]);// 단어 입력 받기 & 제한 시간이 지나면 넘어가기
			...
			while (1)
			{
				...
				else if (result == 2) // 메뉴에서 돌아왔을때
				{
					...
					result = game(grade, timelimit, input, word, scorearray[E][Q_SOLVED]);
				}
				else
					break;
				draws("초등", grade, num);
