...
void elementary(int n) // elementary(0): 처음시작, elementary(1): 처음아님.
{
	...
	for (grade = 1; grade <= 1; grade++)
	{
		...
		for (int num = 1; num <= 3; num++)
		{
			...
			result = game(timelimit, input, word, scorearray[E][Q_SOLVED],E,0,score);// 단어 입력 받기 & 제한 시간이 지나면 넘어가기
			...
			while (1)
			{
				...
				else if (result == 2) // 메뉴에서 돌아왔을때
				{
					...
					result = game(timelimit, input, word, scorearray[E][Q_SOLVED],E,0,score);
				}
				else if (result == 4)
				{
					gobackstart = 1;
