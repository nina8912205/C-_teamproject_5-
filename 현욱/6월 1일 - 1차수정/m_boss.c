...
void m_boss()
{
	...
	Sleep(1000);
	gotoxy(32, 17);	printf("중학교 교육과정을 무사히 마쳤습니다");
	Sleep(1000);
	gotoxy(32, 19);	printf("중학교 교장 선생님을 이기세요");
	Sleep(2000);
	system("cls");
	...
	for (int i = 1; i <= 1; i++)
	{
		...
		drawitem();
		drawmenu();
		scorearray[M][B_Q_SOLVED]++;
		drawtopbar();
		gotoxy(76, 10);	printf("│ SCORE│ %010d", TotalScore);
		gotoxy(25, 10);	printf(" 중학교 졸업 시험 - 문제 %d 번\n", i);

		// 문제 출제
		gotoxy(32, 18);	printf("문제 : ");
		word_short(word);
		gotoxy(70, 17);	printf("배점 : %d 점", M_Bscore);
		start = time(0);

		// 문제 맞추기 & 결과
		gotoxy(32, 19);	printf("문제를 따라 치세요 : ");
		result = game(M_Boss_time, input, word, scorearray[M][B_Q_SOLVED],M,1,M_Bscore);
		end = time(0);
		...
		if (result) {
			...
			if (O_X[scorearray[M][B_Q_SOLVED] - 2] == 1)
				boss_combo(M);*/

			combo[i - 1] = 1;
			O_X[scorearray[M][B_Q_SOLVED] - 1] = 1;
			scorearray[M][B_Q_CORRECT]++;
		}
		else {
			combo[i - 1] = 0;
			O_X[scorearray[M][B_Q_SOLVED] - 1] = 0;
		}
	}
}