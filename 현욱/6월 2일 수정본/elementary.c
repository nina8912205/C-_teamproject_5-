...
void elementary(int n)
{
	...
	extern int SCORE[SCHSIZE][ASIZE], TotalScore;
	...
	for (grade = 1; grade <= 6; grade++)
	{
		for (int num = 1; num <= 2; num++)
		{
			...
			/*gotoxy(32, 24);	printf("total: %d, e_time: %d", TotalScore, SCORE[E][TIME]);
			gotoxy(32, 25);	printf("e_correct: %d, e_combo: %d", SCORE[E][CORRECT], SCORE[E][COMBO]);*/
